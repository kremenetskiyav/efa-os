...
## Production Daily Commercial Brief Delivery v1 — inactive
...
The n8n workflow `Ozon Daily Commercial Brief Delivery v1`
(`Kf241Y5kzETghygL`) is retained but inactive as of 2026-08-22. Its former
08:15 Europe/Moscow schedule no longer sends Email or Telegram. Before
retirement it obtained the
deterministic brief, sent the existing HTML/PDF representation through Gmail,
and sent the compact Telegram representation through the existing Telegram
...
Both production channel paths previously completed a controlled manual E2E:
Gmail OAuth2 delivered the verified HTML plus PDF, and Telegram delivered the
approved compact summary to the confirmed private numeric chat. This delivery
path is now retired from the automatic schedule.
...
a five-page Cyrillic-safe PDF. Daily Brief delivery is inactive, while generated
previews and all credential material remain outside Git.

## AI Analyst compact delivery — active

AI Analyst remains generated by the existing read-only cron at 16:00
Europe/Moscow. At 16:30 the existing delivery cron formats the latest report
once and posts it to `EFAANALYSTEMAIL`. That active workflow sends the same
compact payload through the existing Gmail and Telegram credentials. The
Telegram text contains the day summary and all five SKU decisions, including
price, recommendation, promotion, PBT, margin and reason. It does not send the
technical TXT report and does not recalculate analytics.
