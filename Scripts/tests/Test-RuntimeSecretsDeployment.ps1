...
foreach ($name in 'EFA_DB_HOST', 'EFA_DB_PORT', 'EFA_DB_NAME', 'EFA_DB_USER', 'EFA_DB_PASSWORD') {
    Assert-True ($initializerSource.Contains($name) -and $deployerSource.Contains($name) -and $exampleSource.Contains("$name=") -and $composeSource.Contains(('${' + $name + '}'))) "Missing runtime contract: $name"
}
